	...
	public function index(){
		...
		if($data['type'] == '1'){ //페이스북 코드 1
			...
			if(!empty($user)){
				...
				//20150209 페북 publish_actions 허가 통과 후 교체
				//facebook image post 기능 
				/* $post_type = 'url';
				if($w != 's'){
					$post_type = 'source';
				...
				try {
					...
					$response = (new FacebookRequest(
							...
							$cdn_url = 'http://2j5xlt4h84.ecn.cdn.infralab.net/data/event/'.$upDate.'/'.$data['uuid'].'_thumb.'.$fname[1];
							$this->session->set_flashdata('apply_img',$cdn_url);
				...
				$this->session->set_flashdata('apply_complete','teaser');
				if($w == 's'){
					$this->session->set_flashdata('apply_img',$pic_path);
				}else{
					$cdn_url = 'http://2j5xlt4h84.ecn.cdn.infralab.net/data/event/'.$upDate.'/'.$data['uuid'].'_thumb.'.$fname[1];
					$this->session->set_flashdata('apply_img',$cdn_url);
				}

				$content = $this->common->conv_content(trim($this->input->post('bf_content', TRUE)),0);
				$this->session->set_flashdata('apply_txt',$content);
							redirect('teaser#snsBtn');
